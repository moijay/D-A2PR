...
class DiffusionModel(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, max_action, device, num_timesteps=5):
        ...
        # 定义β调度
        self.betas = torch.linspace(1e-4**0.5, 0.02**0.5, num_timesteps,dtype=torch.float32,device=device)**2
    ...
    def forward(self, state, action, t=None):
        ...
        # 添加噪声到动作
        sqrt_alpha_cumprod_t = torch.sqrt(self.alphas_cumprod[t])[:, None]
        sqrt_one_minus_alpha_cumprod_t = torch.sqrt(1. - self.alphas_cumprod[t])[:, None]
        
        noise = torch.randn_like(action)
        noisy_action = sqrt_alpha_cumprod_t * action + sqrt_one_minus_alpha_cumprod_t * noise
        
        # 预测噪声
        predicted_noise = self.model(torch.cat([state, noisy_action], dim=1))
    ...
    def sample(self, state):
        ...
        for t in reversed(range(self.num_timesteps)):
            ...
            # 预测噪声
            predicted_noise = self.model(torch.cat([state, x_t], dim=1))
            ...
            ) + torch.sqrt(beta_t) * (1-self.alphas_cumprod[t-1])/(1-self.alphas_cumprod[t])*noise
